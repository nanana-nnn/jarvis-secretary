...
1. ファイアウォールを開ける。**開けるのは LAN からの 5173 と 8787 だけ。**
   ...
   ```
2. スマホで `https://<HOST>:5173` を開き、証明書を信頼する
3. **マイクを有効にする**を1回押す（iOS はユーザー操作なしにマイクを開けない）
4. ホーム画面に追加すると全画面で常設できる
5. 指を1回鳴らす。コアが反応すれば通し（既定は指パッチン1回。DESIGN.md §7）
...
**このサーバーは接続してきた端末を認証しない。** 効いているのは CORS と WebSocket の
Origin チェックだけで、これは同じ Wi-Fi にいる相手には壁にならない（Origin は詐称できる）。
つまり **LAN 内の誰でも、話しかけずに `/jobs/{id}/approve` を叩ける。**

DESIGN.md §13 にある QR ペアリングと端末トークンは**まだ実装されていない**。

- 信用できる人しかいない Wi-Fi で使う
- 公共・共有の Wi-Fi では使わない
- インターネットへ出さない
