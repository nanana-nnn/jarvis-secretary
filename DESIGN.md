...
- [ ] iPhone で字幕表示する
- [ ] 回答は上段の固定高ターミナルカードへ文字表示する（読み上げは使わない）
...
### 次の実装：文字回答カード（2026-09-02 合意）

- 回答の音声読み上げは廃止する
- 質問後はUI上段を、Starship風の丸いバッジを持つターミナルカードへ切り替える
- 質問は `› 質問文`、処理中は点滅カーソル、回答は段落または行単位で追加表示する
- カードの高さは固定する。出力中は最下部へ自動追従し、古い文字は上側へ見切れる
- 利用者が途中で上へスクロールしたら自動追従を止める
- 全文表示後はカード内を自由にスクロールできる
- 完了後に「戻る」「続けて聞く」の2ボタンを表示する。拍手を戻る操作には兼用しない
- 聞き取り開始から回答表示完了までは、カードの縁・背景・外側のぼかしを
  壁紙テーマ色で約2.5秒周期に明暗させ、呼吸するように見せる。回転する縁は使わない
- 完了時は呼吸を止め、弱い常時発光へ落ち着かせる
