...
- `secretary-sleep.webp`：同一画角の閉眼素材
  ...
  > **2026-08-30 更新。** 新素材は実質同じ16:9で、グレースケール RMSE の最小は
  > オフセット (0, 0)。書き出し時に両方を 1672×940 へ揃える。
  > 目の位置が前世代の約62%から左側（awake 約35%）へ移ったため、
  > `object-position` は共通の `38% 44%` を初期値とする。
  > 素材差し替え時は毎回同じ位置合わせを検証すること。
