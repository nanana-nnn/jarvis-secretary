...
| `SLEEP` | コアが小さく呼吸し、輝度を落とす | 指パッチだけ監視 |
| `WAKING` | コアから衝撃波 | 短い起動音 |
| `LISTENING` | コアが声量と高周波に追従 | 「はい、どうしました？」 |
| `TRANSCRIBING` | コアが収束し、字幕を順次表示 | なし |
| `THINKING` | コア内部が回路状に変化 | 任意の小さな待機音 |
| `APPROVAL` | 左側に変更要約と承認／却下 | 「確認してください」 |
| `SPEAKING` | コアが中心から外へ波打ち、回答字幕を表示 | 回答読み上げ |
| `ERROR` | コアがエラー色へ変化し、原因と再試行を表示 | 短いエラー音 |
| `OFFLINE` | コアが点へ縮退し「PCと未接続」 | 指パッチ検出のみ継続 |
...
| `SLEEP` | 指パッチ1回を検出 | `WAKING` |
| `WAKING` | 衝撃波アニメ完了（250ms）＋起動発話 | `LISTENING` |
...
**既定は指パッチ1回。** iPhone実機での検出ログを基に、高周波比と立ち上がりを組み合わせて判定する。
...
- `CLAP_MODE=single` では条件を満たした1回目で起動する
- `double` は調整用に残し、1回目から `CLAP_GAP_MIN`〜`CLAP_GAP_MAX` の間に2回目があれば起動する
...
| `CLAP_MODE` | `single` | `double` / `single` |
...
iPhone は縦置きを正とする。秘書画像ではなく、CachyOS の `sysmon-dots.py` と同じ発想の音声反応ピクセルコアをJARVISの顔にする。

- 中央に33×33セルの左右対称なピクセルコアを置く。丸い一般的なAIオーブにはしない
- 同じマイクストリームの `rms` と `hfRatio` を描画へ分岐し、指パッチン検出とは独立に60fpsで反応させる
- `SLEEP` は小さく呼吸、`WAKING` は衝撃波、`LISTENING` は声量と高周波へ追従する
- `TRANSCRIBING` は収束、`THINKING` は回路状の内部変化、`SPEAKING` は中心から外への波、`OFFLINE` は点へ縮退する
