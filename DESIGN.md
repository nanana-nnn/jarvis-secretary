...
3. 指を1回鳴らすとコアが衝撃波を返し、「なにする？」を Vault へ即座に問う（起動方式の真値は §7）
...
- 壁紙切り替え時に caelestia が配色生成中の一時的な例外を返した場合は、0.8秒待って
  1回だけ再試行する。画像固有の失敗と区別できるよう、最終失敗はファイル名と例外末尾をログに残す
...
- 本文の生成（Codex）とは分けてある。失敗したときに原因が本文側かブラウザ側かを
  切り分けられるようにするため
- JARVIS本体から呼ぶCodexは `visible=True` の常駐ターミナルへ流す。noteの本文生成も
  PC画面に作業を表示したまま、完了後にブラウザ操作へ進む
