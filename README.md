...
> 🚧 **Work in Progress — まだ完成途中ですが公開します。**
> 実際に触った人の意見をもらいながら育てたいです。
> 特に UI / 使い方 / 「ここ分かりにくい」みたいな感想を歓迎します。見つけたら Issues か Discussions へどうぞ。
>
> - 今できてること: 指パッチンで起動 → 音声で聞く → Vault を読んで答える → 書き込みは承認制（Phase 0–4）
> - 未完成なところ: 常設品質（Phase 5: OLED 保護・自動再起動・停電復帰）は未着手。一部は実機未確認
> - 意見が欲しいポイント:
>   1. UI — 常設端末としての見やすさ・文字サイズ・操作感
>   2. 使い方・導入 — SETUP.md の分かりにくさ、つまずいた点
>   3. 起動・聞き取りの体感 — 誤起動の多さ、聞き間違いのパターン
...
| **端末は登録制** | 初回だけ PC で合図を出して登録する。登録していない端末は繋がらない |
...
- **登録した端末だけが繋がる。** 初回に PC で合図を出して1回だけ登録する。
  トークンの実値はサーバーに残さない（SHA-256 と `hmac.compare_digest`）。範囲は [SETUP.md §11](SETUP.md#11-認証の範囲設置前に読む)
